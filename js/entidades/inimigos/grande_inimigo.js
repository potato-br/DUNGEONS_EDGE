...
function criarGrandeInimigoEmPlataforma(platform) {
    ...
    return {
        ...
        _slimeLastSound: performance.now() // timer para som periódico
    };
}
...
function updateGrandeInimigos() {
    for (let i = grandeInimigos.length - 1; i >= 0; i--) {
        ...
        // Som periódico a cada 1s, apenas se o slime estiver visível na tela
        if (typeof AudioManager !== 'undefined' && AudioManager && typeof AudioManager.play === 'function') {
            const now = performance.now();
            if (!e._slimeLastSound) e._slimeLastSound = now;
            // Checa se o slime está visível na tela
            const visivel = (
                e.x + e.width > 0 && e.x < (typeof gamePlayArea !== 'undefined' ? gamePlayArea.x + gamePlayArea.width : (typeof screenWidth !== 'undefined' ? screenWidth : 9999)) &&
                e.y + e.height > 0 && e.y < (typeof screenHeight !== 'undefined' ? screenHeight : 9999)
            );
            if (visivel && now - e._slimeLastSound >= 1000) {
                try { AudioManager.play('slime_sfx', { volume: 0.10 }); } catch (err) {}
                e._slimeLastSound = now;
            }
        }
        ...
        if (e.y > screenHeight + 300) {
            // removido som de morte para offscreen (não tocar sfx quando slime sai da tela)
