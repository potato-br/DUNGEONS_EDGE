...
function spawnMoeda(forceRainCoin = false) {
  ...
  const tutorialPaused = (typeof DungeonTutorial !== 'undefined' && (DungeonTutorial._tutorialPaused || DungeonTutorial._tutorialMovementOnly));
  if (isPaused || tutorialPaused || document.getElementById('blackScreenTransition')) return;
...
function updateMoedas() {
  // Não atualiza se estiver pausado ou em pausa do tutorial, mas continua se for gameover e tiver moedas caindo
  const tutorialPaused = (typeof DungeonTutorial !== 'undefined' && (DungeonTutorial._tutorialPaused || DungeonTutorial._tutorialMovementOnly));
  if (isPaused || tutorialPaused || (gameState !== 'jogando' && !moedas.some(m => m.isFalling))) return;
...
function checkMoedaCollision() {
  ...
  moedas.forEach(m => {
    if (!m.collected && rectIntersect(player, m)) {
      ...
      try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('coin_sfx'); } catch (err) {}
