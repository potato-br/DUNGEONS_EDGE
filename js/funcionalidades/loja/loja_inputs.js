canvas.addEventListener('click', function(e) {
  ...
  if (insufficientFundsMessage) {
    // Allow clicking the overlay close 'X'
    try {
      const rect = canvas.getBoundingClientRect();
      const mx = (e.clientX - rect.left) * (canvas.width / rect.width);
      const my = (e.clientY - rect.top) * (canvas.height / rect.height);
      if (typeof insufficientFundsCloseRect !== 'undefined' && insufficientFundsCloseRect && mx >= insufficientFundsCloseRect.x && mx <= insufficientFundsCloseRect.x + insufficientFundsCloseRect.w && my >= insufficientFundsCloseRect.y && my <= insufficientFundsCloseRect.y + insufficientFundsCloseRect.h) {
        try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
        try { if (typeof closeInsufficientFunds === 'function') closeInsufficientFunds(); } catch (err) {}
      }
    } catch (err) {}
    return;
  }
  
  // Tutorial bloqueio: bloqueia cliques durante todo o tutorial, exceto em certas fases
  try {
    if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial) {
      // storeBuyingIntroduction: bloqueia todos os cliques (apenas hover/focus permitido)
      if (Tutorial._stage === 'storeBuyingIntroduction') {
        e.preventDefault();
        e.stopPropagation();
        return;
      }

      // storeBuyingPurchase: allow clicks only when targeting the marked item
      if (Tutorial._stage === 'storeBuyingPurchase') {
          ...
          const my = (e.clientY - rect.top) * (canvas.height / rect.height);
          let allowed = false;
          const marked = (Tutorial._markedItem && typeof Tutorial._markedItem.itemName === 'string') ? Tutorial._markedItem.itemName : null;
          if (marked && Array.isArray(lojaOptionRects)) {
            for (const r of lojaOptionRects) {
              if (!r) continue;
              if (mx >= r.x && mx <= r.x + r.w && my >= r.y && my <= r.y + r.h) {
                const visibleItems = shopItems.filter(item => isItemVisible(item));
                const item = visibleItems[r.index];
                if (item && item.nome === marked) { allowed = true; break; }
              }
            }
          }
          if (!allowed) {
      ...
      // Outras fases: bloqueia tudo exceto dungeonInteraction e failedDepth50
  ...
  // Tutorial bloqueio: em stage 'dungeonInteraction', apenas o botão Dungeon pode ser clicado
  try {
    if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && (Tutorial._stage === 'dungeonInteraction' || Tutorial._stage === 'failedDepth50')) {
      const { x: dungeonX, y: dungeonY, w: dungeonW, h: dungeonH } = getDungeonBtnRect();
      const rect = canvas.getBoundingClientRect();
      const mx = (e.clientX - rect.left) * (canvas.width / rect.width);
      const my = (e.clientY - rect.top) * (canvas.height / rect.height);
      if (!(mx >= dungeonX && mx <= dungeonX + dungeonW && my >= dungeonY && my <= dungeonY + dungeonH)) {
        // Clique não está no botão Dungeon - bloqueia
        e.preventDefault();
        e.stopPropagation();
        return;
      }
  ...
  try { if (typeof previewsRead !== 'undefined' && pr && pr.nome) previewsRead.add(pr.nome); } catch (e) {}
        // Tutorial: notificar interação com preview (apenas na fase 'select')
        try { if (typeof Tutorial !== 'undefined' && typeof Tutorial.onPreviewInteracted === 'function' && Tutorial._stage === 'previewSelect') Tutorial.onPreviewInteracted(); } catch (e) {}
        drawLoja();
        found = true;
        break;
      }
  ...
  if (mx >= dungeonBtnX && mx <= dungeonBtnX + dungeonBtnW &&
    ...
    try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
    // Tutorial dungeonInteraction: detectar clique no botão Dungeon
    if (typeof gameState !== 'undefined' && gameState === 'loja' && typeof Tutorial !== 'undefined' && (Tutorial._stage === 'dungeonInteraction' || Tutorial._stage === 'failedDepth50')) {
      try { if (window.OptionsMenu && typeof window.OptionsMenu.close === 'function') window.OptionsMenu.close(); } catch (e) {}
      closeShop();
      return;
    }
  ...
  for (let i = 0; i < lojaOptionRects.length; i++) {
    ...
    if (mx >= r.x && mx <= r.x + r.w && my >= r.y && my <= r.y + r.h) {
      ...
      const item = visibleItems[r.index];
      if (item) {
        // mark as seen/read (unified). Persist into _charData.__global.seenShopItems
        try {
...
canvas.addEventListener('mousemove', function(e) {
  ...
  // Tutorial: em stage 'preview-select' ou 'storeBuyingIntroduction' (e explanation), bloqueia interação enquanto digitando
  try {
    if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && (Tutorial._stage === 'previewSelect' || Tutorial._stage === 'storeBuyingIntroduction' || Tutorial._stage === 'storeBuyingexplanation')) {
      // Se ainda está digitando, bloqueia tudo
      if (Tutorial._typing && Tutorial._typing.isTyping) {
        return;
      }

      // If we're in the explanation stage, block all navigation (no mouse/keyboard navigation)
      if (Tutorial._stage === 'storeBuyingexplanation') {
        return;
      }

      // Apenas permite interação com previews/items, bloqueia outros elementos
      const rect = canvas.getBoundingClientRect();
      const mx = (e.clientX - rect.left) * (canvas.width / rect.width);
      const my = (e.clientY - rect.top) * (canvas.height / rect.height);

      // Para previewSelect: verifica se está hovering um preview
      if (Tutorial._stage === 'previewSelect') {
        ...
        if (!isOverPreview) {
          // Se não está sobre preview, processa hover normalmente mas sem efeito visual de botões
          // Permite que o hover visual aconteça mas bloqueia seleção de items
          const rect2 = canvas.getBoundingClientRect();
          const mx2 = (e.clientX - rect2.left) * (canvas.width / rect2.width);
          const my2 = (e.clientY - rect2.top) * (canvas.height / rect2.height);

          // Limpa seleção de items mas permite hover visual
          try { selectedIndex = -1; } catch (e) {}
          drawLoja();
          return;
        }
      ...
      // Para storeBuyingIntroduction: apenas permite hover sobre Cinto Relâmpago
      if (Tutorial._stage === 'storeBuyingIntroduction') {
        let isOverCinto = false;
        for (const r of lojaOptionRects) {
          if (r && mx >= r.x && mx <= r.x + r.w && my >= r.y && my <= r.y + r.h) {
            const visibleItems = shopItems.filter(item => isItemVisible(item));
            const item = visibleItems[r.index];
            if (item && item.nome === 'Cinto Relâmpago') {
              isOverCinto = true;
              break;
            }
          }
        ...
        if (!isOverCinto) {
          return; // Bloqueia hover fora do Cinto Relâmpago
        }
      }
    }
  } catch (err) {}
...
// Track hover over insufficient-funds close button when overlay is active
canvas.addEventListener('mousemove', function(e) {
  ...
  try {
    ...
    const mx = (e.clientX - rect.left) * (canvas.width / rect.width);
    const my = (e.clientY - rect.top) * (canvas.height / rect.height);
    const r = typeof insufficientFundsCloseRect !== 'undefined' ? insufficientFundsCloseRect : null;
    const prev = typeof insufficientCloseHovered !== 'undefined' ? insufficientCloseHovered : false;
    if (r && mx >= r.x && mx <= r.x + r.w && my >= r.y && my <= r.y + r.h) {
      insufficientCloseHovered = true;
    } else {
      insufficientCloseHovered = false;
    }
    if (insufficientCloseHovered !== prev) drawLoja();
  } catch (err) {}
});

canvas.addEventListener('mouseout', function(e) {
  if (typeof insufficientCloseHovered !== 'undefined' && insufficientCloseHovered) {
    insufficientCloseHovered = false;
...
window.addEventListener('keydown', e => {
  ...
  if (insufficientFundsMessage) {
    // Allow Enter/Space/Escape to close the insufficient funds overlay via keyboard
    try {
      const k = e.key ? e.key.toLowerCase() : '';
      if (k === 'enter' || k === ' ' || k === 'space' || k === 'spacebar' || k === 'escape') {
        try { if (typeof closeInsufficientFunds === 'function') closeInsufficientFunds(); } catch (err) {}
  ...
  try {
        ...
        // Confirm selection by keyboard: either close modal or activate character
        try {
          const unlockedChars = (typeof getUnlockedCharacters === 'function') ? getUnlockedCharacters() : [];
          const totalChars = unlockedChars.length;
          if (typeof selectedCharacterModalIndex === 'number') {
            if (selectedCharacterModalIndex === totalChars) {
              try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
              showCharacterSelect = false;
              try { modalPreviewKeyboardLock = null; } catch (e) {}
              modalPreviewSelected = null;
              ...
              e.preventDefault(); e.stopPropagation();
              return;
            } else if (selectedCharacterModalIndex >= 0 && selectedCharacterModalIndex < totalChars) {
              const nome = unlockedChars[selectedCharacterModalIndex];
              try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
              try { setActiveCharacter(nome); } catch (err) {}
              showCharacterSelect = false;
              try { modalPreviewKeyboardLock = null; } catch (e) {}
              modalPreviewSelected = null;
              hoveredPreview = null;
  ...
  const visibleItems = shopItems.filter(item => isItemVisible(item));
      ...
      if (!showCharacterSelect && gameState === 'loja') {
        ...
        if (e.key === 'ArrowLeft' || keyLower === 'a' || e.key === 'ArrowRight' || keyLower === 'd') {
          ...
          if (selectedElement && (selectedElement.type === 'dungeon' || selectedElement.type === 'character' || selectedElement.type === 'settings')) {
            ...
            if (activePreview) {
              ...
              // Notify tutorial of preview interaction during preview-select stage
              try { if (typeof Tutorial !== 'undefined' && typeof Tutorial.onPreviewInteracted === 'function' && Tutorial._stage === 'previewSelect') Tutorial.onPreviewInteracted(); } catch (e) {}
              modalPreviewSuppressUntilMouseMove = true;
              modalPreviewMouseActive = false;
              isDungeonButtonHovered = false;
        ...
        if ((e.key === 'ArrowUp' || keyLower === 'w' || e.key === 'ArrowDown' || keyLower === 's') && selectedElement && selectedElement.type === 'preview') {
          try {
            ...
            modalPreviewSuppressUntilMouseMove = true;
            modalPreviewMouseActive = false;
            // Notify tutorial of preview interaction when navigating with keyboard during preview-select stage
            try { if (typeof Tutorial !== 'undefined' && typeof Tutorial.onPreviewInteracted === 'function' && Tutorial._stage === 'previewSelect') Tutorial.onPreviewInteracted(); } catch (e) {}
            try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
            drawLoja();
            e.preventDefault(); e.stopPropagation();
    ...
    if (e.key === 'Enter') {
      // Tutorial: bloqueio de Enter apenas durante preview-select
      try {
        if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && Tutorial._stage === 'previewSelect') {
          return;
        }
      } catch (err) {}
      
      // Tutorial: durante 'dungeonInteraction' apenas o botão Dungeon pode ser confirmado
      try {
        if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && (Tutorial._stage === 'dungeonInteraction' || Tutorial._stage === 'failedDepth50')) {
          if (selectedElement && selectedElement.type === 'dungeon') {
            try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
            try { if (window.OptionsMenu && typeof window.OptionsMenu.close === 'function') window.OptionsMenu.close(); } catch (e) {}
            closeShop();
            return;
          } else if (selectedElement && (selectedElement.type === 'settings' || selectedElement.type === 'character' || selectedElement.type === 'items')) {
            // Bloqueia Enter para qualquer outro botão ou item
            e.preventDefault();
            e.stopPropagation();
            return;
          }
  ...
  switch (selectedElement.type) {
        ...
        case 'items':
          if (!showCharacterSelect) {
            try { if (typeof AudioManager !== 'undefined' && AudioManager.play) AudioManager.play('select_sfx'); } catch (err) {}
            // If tutorial requires buying a marked item, only allow Enter when selected matches
                try {
                  if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && Tutorial._stage === 'storeBuyingPurchase') {
                    const visibleItems = shopItems.filter(item => isItemVisible(item));
                    const item = visibleItems[selectedIndex];
                    const markedName = (Tutorial._markedItem && typeof Tutorial._markedItem.itemName === 'string') ? Tutorial._markedItem.itemName : null;
                    if (!item || !markedName || item.nome !== markedName) {
                      e.preventDefault(); e.stopPropagation();
                      break;
                    }
                  }
...
canvas.addEventListener('mousemove', function(e) {
    if (gameState !== 'loja' || showCharacterSelect) return;
    // block mouse navigation during storeBuyingexplanation
    try { if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && Tutorial._stage === 'storeBuyingexplanation') return; } catch (err) {}
...
window.addEventListener('keydown', e => {
    if (gameState !== 'loja' || showCharacterSelect) return;
    // block keyboard navigation during storeBuyingexplanation
    try { if (typeof Tutorial !== 'undefined' && Tutorial.showShopTutorial && Tutorial._stage === 'storeBuyingexplanation') { e.preventDefault(); e.stopPropagation(); return; } } catch (err) {}
