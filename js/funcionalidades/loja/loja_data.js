...
// Unified set: tracks items that have been seen/read in the shop (both 'NEW' and 'LEIA')
let seenShopItems = new Set();
let shopMessage = '';
...
let isSettingsButtonHovered = false;
let lojaZooms = [];
let lojaWobbles = [];
...
// allow these to be re-bound after loading saves
let globalPurchases = _charData.__global.purchases;
let globalRevealedItems = _charData.__global.revealedItems;

function rebindGlobalLojaState() {
  try {
    if (!_charData.__global) {
      _charData.__global = { purchases: {}, revealedItems: {} };
    }
    globalPurchases = _charData.__global.purchases = _charData.__global.purchases || {};
    globalRevealedItems = _charData.__global.revealedItems = _charData.__global.revealedItems || {};
    // Restore seen/read shop items from saved _charData.__global if present
    try {
      const saved = _charData.__global.seenShopItems || {};
      seenShopItems = new Set(Object.keys(saved));
    } catch (e) {
      seenShopItems = new Set();
    }
  } catch (e) {
    // fail silently
  }
...
const shopItems = [
  ...
  },
  {
    ...
    descricao: 'Sim, deixar suas moedas invisíveis… exatamente o que você precisava.... pra continuar pobre, mas agora de forma mais misteriosa. aumenta +85 o valor das moedas',
    preco: 10000,
    priceMultiplier: 2,
    priceIncrement: 0,
    requiredDepthSteps: {
  ...
  },
  {
    ...
    descricao: 'Coloque na cabeça e sinta imediatamente vontade de dar nomes sofisticados para suas moedas “Este é o Senhor Centavo”. aumenta +285 o valor das moedas',
    preco: 40000,
    priceMultiplier: 2,
    disponivel: true,
    priceIncrement: 0,
...
const SECRET_ITEMS = [
  ...
  },
  {
    ...
    efeito: () => {
      ...
      _charData.__global.savedStartItemName = 'Escada Grande';
        try { 
              // seleciona aleatoriamente uma das três músicas para high-depth (pending for next run)
              if (typeof window !== 'undefined') {
                const choices = ['gameplay_music', 'gameplay_music_1', 'gameplay_music_2'];
                try { window.__highDepthSelectionPending = choices[Math.floor(Math.random() * choices.length)]; } catch (e) { window.__highDepthSelectionPending = null; }
              }
            } catch (e) {}
    }
  },
  ...
  _charData.__global.savedStartItemName = 'Gárgula Fofinha';
    try { if (typeof window !== 'undefined') { const choices = ['gameplay_music', 'gameplay_music_1', 'gameplay_music_2']; try { window.__highDepthSelectionPending = choices[Math.floor(Math.random()*choices.length)]; } catch (e) { window.__highDepthSelectionPending = null; } } } catch (e) {}
    }
  },
  {
