...
var AudioManager = (function () {
  ...
  a.preload = 'auto';
  a.loop = !!opts.loop;
  // per-asset base volume (relative scale). For music this will be multiplied
  // by the global musicVolume so per-track defaults (like 0.8) are preserved
  // while still following the user-set musicVolume.
  a.__baseVolume = (typeof opts.volume === 'number') ? Number(opts.volume) : 1;
  // Consider an asset to be 'music' using this precedence:
  // 1) explicit opts.isMusic (caller-provided)
  // 2) explicit opts.type === 'music'
  // 3) if loop is true (common for music/ambients)
  // 4) name-based heuristic (fallback)
  if (typeof opts.isMusic === 'boolean') {
    a.__isMusic = opts.isMusic;
  } else if (opts.type === 'music') {
    a.__isMusic = true;
  } else {
    a.__isMusic = !!opts.loop || (/music|ambience|bgm|theme|background/i).test(name);
  }
  // initial effective volume: music uses musicVolume * base, sfx uses sfxVolume * base
  try { a.volume = a.__isMusic ? (musicVolume * (a.__baseVolume || 1)) : (sfxVolume * (a.__baseVolume || 1)); } catch (e) {}
  assets[name] = a;
  return a;
  }
  ...
  function stop(name) {
    ...
    if (!a) return;
    try {
      // If the asset has a source URL, also try to stop any cloned Audio elements
      // that were created via `new Audio(src)` and stored in window._activeSerraSounds.
      try {
        const assetSrc = (a && (a.src || a.currentSrc || (a.getAttribute && a.getAttribute('src')))) || null;
        if (assetSrc && typeof window !== 'undefined' && Array.isArray(window._activeSerraSounds)) {
          for (let i = window._activeSerraSounds.length - 1; i >= 0; i--) {
            const el = window._activeSerraSounds[i];
            try {
              const elSrc = (el && (el.src || el.currentSrc || (el.getAttribute && el.getAttribute('src')))) || null;
              if (elSrc && (elSrc === assetSrc || elSrc.endsWith(assetSrc) || assetSrc.endsWith(elSrc))) {
                try { el.pause(); el.currentTime = 0; } catch (er) {}
                try { window._activeSerraSounds.splice(i, 1); } catch (er) {}
              }
            } catch (e) {}
          }
        }
      } catch (e) {}
      // Always stop the registered asset element as well
  ...
  if (typeof isDebugMode !== 'undefined' && isDebugMode) console.log('[AudioManager] playMusic called:', name, options, { musicAudio });
    ...
    if (musicAudio && musicAudio !== assets[name]) {
      ...
      if (!muted) {
        ...
        // fade out old and fade in new. Target fade-in volume should respect
        // per-track baseVolume
        fadeVolume(old, 0, duration, () => { try { old.pause(); old.currentTime = 0; } catch (e) {} });
        fadeVolume(next, musicVolume * (next.__baseVolume || 1), duration);
      }
      else {
  ...
  musicAudio.loop = (typeof options.loop === 'boolean') ? options.loop : true;
    try { musicAudio.volume = musicVolume * (musicAudio.__baseVolume || 1); } catch (e) {}
    if (!muted) play(name);
  }
  ...
  if (musicAudio.paused) {
          try {
            ...
            try {
              ...
              const start = performance.now();
              const from = 0;
              // restore to per-track scaled music volume
              const to = musicVolume * (musicAudio.__baseVolume || 1);
              const dur = 300;
              const step = () => {
                if (musicAudio.__fadeToken !== token) {
                  musicAudio.__isFading = false;
  ...
  function setMusicVolume(v) {
    ...
    try {
      Object.keys(assets).forEach(k => {
        ...
        if (!a) return;
        if (a.__isMusic) {
          try { a.volume = musicVolume * (a.__baseVolume || 1); } catch (e) {}
        }
      });
    } catch (e) {}
  // also ensure currently playing music reflects the new global volume
  try { if (musicAudio) musicAudio.volume = musicVolume * (musicAudio.__baseVolume || 1); } catch (e) {}
  }
  function setSfxVolume(v) {
    sfxVolume = Number(v);
    if (Number.isNaN(sfxVolume)) sfxVolume = 1;
    try {
      Object.keys(assets).forEach(k => {
        const a = assets[k];
        if (!a) return;
        // apply to non-music assets (SFX/one-shots)
        if (!a.__isMusic) {
          try { a.volume = sfxVolume * (a.__baseVolume || 1); } catch (e) {}
        }
      });
    } catch (e) {}
  }
  ...
  function setDefaultMusicVolume(v) {
    DEFAULT_MUSIC_VOLUME = Number(v) || DEFAULT_MUSIC_VOLUME;
  ...
  function playWithFade(name, fadeOutMs = 300, options = {}) {
    ...
    try {
      ...
      a.loop = !!options.loop;
      // allow override volume for this play. Interpret options.volume as a relative
      // multiplier (1 = full global volume) so it will respect music/sfx sliders.
      try {
        const base = asset && (asset.__baseVolume || 1);
        const global = asset && asset.__isMusic ? musicVolume : sfxVolume;
        if (typeof options.volume === 'number') {
          // treat provided volume as a relative multiplier to the global level
          a.volume = Math.max(0, Math.min(1, (global * base * Number(options.volume))));
        } else {
          a.volume = Math.max(0, Math.min(1, (global * base)));
        }
      } catch (e) {}
      // track transient cloned elements so stop() can find them by src
      try {
  ...
  function playAmbient(name, options = {}) {
    ...
    try {
      a.loop = true;
      // If caller provides a volume, interpret it as a relative multiplier and
      // combine it with the global music/sfx volume so ambient layers respond
      // to the user's sliders. If no volume provided, fall back to global*base.
      try {
        const base = a.__baseVolume || 1;
        const global = a.__isMusic ? musicVolume : sfxVolume;
        if (typeof options.volume === 'number') {
          a.volume = Math.max(0, Math.min(1, global * base * Number(options.volume)));
        } else {
          a.volume = Math.max(0, Math.min(1, global * base));
  ...
  load('menu_music', 'media/audio/menu_music.ogg', { loop: false, volume: 1, isMusic: true });
    // Put your audio files in media/audio/ and name accordingly.
  load('shop_music', 'media/audio/shop_music.ogg', { loop: true, volume: 0.8, isMusic: true });
  // game over cue is a one-shot SFX (should follow SFX slider)
  load('gameover_sfx', 'media/audio/gameover_sfx.ogg', { loop: false, volume: 0.3, isMusic: false });
  ...
  load('gameplay_music', 'media/audio/gameplay_music.ogg', { loop: true, volume: 0.5, isMusic: true });
  load('gameplay_music_1', 'media/audio/gameplay_music_1.ogg', { loop: true, volume: 0.5, isMusic: true });
  load('gameplay_music_2', 'media/audio/gameplay_music_2.ogg', { loop: true, volume: 0.5, isMusic: true });
    // spider sound effect
  ...
  // dash sound (used by Errante and Ninja)
    ...
    load('platform_break_sfx', 'media/audio/platform_break_sfx.ogg', { loop: false, volume: 0.25 });
  ...
  load('ambience', 'media/audio/ambience.ogg', { loop: true, volume: 0.1, isMusic: true });
  // shop ambient
  load('shop_ambience', 'media/audio/shop_ambience.ogg', { loop: true, volume: 0.3, isMusic: true });
