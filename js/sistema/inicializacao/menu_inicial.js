function setupMenuInicial(onStart) {
    ...
    let carregarBtn = null;
        try {
            let lastSlot = Number(localStorage.getItem('dungeons_edge_last_slot')) || null;
            const findSaveSlot = async () => {
                // prefer lastSlot if present
                if (lastSlot) {
                    if (window.SaveManager && typeof SaveManager.loadSlot === 'function') {
                        try { const s = await SaveManager.loadSlot(lastSlot); if (s) return lastSlot; } catch (e) {}
                    }
                    try { if (localStorage.getItem(`dungeons_edge_save_slot_${lastSlot}`)) return lastSlot; } catch (e) {}
                }
                // fallback: scan slots 1..3 and return first found
                for (let i = 1; i <= 3; i++) {
                    try {
                        if (window.SaveManager && typeof SaveManager.loadSlot === 'function') {
                            try { const s = await SaveManager.loadSlot(i); if (s) return i; } catch (e) {}
                        }
                        try { if (localStorage.getItem(`dungeons_edge_save_slot_${i}`)) return i; } catch (e) {}
                    } catch (e) {}
                }
                return null;
            };
    ...
    slotBtn.classList.add('overlay-button');
        ...
        slotBtn.addEventListener('click', function() {
            (async () => {
                ...
                try { if (!exists) { const raw = localStorage.getItem(`dungeons_edge_save_slot_${i}`); if (raw) exists = true; } } catch (e) {}
                ...
                const doSave = async () => {
                    try {
                        ...
                        try { if (!existing) { const raw = localStorage.getItem(`dungeons_edge_save_slot_${i}`); if (raw) existing = JSON.parse(raw); } } catch (e) { existing = existing || null; }
                        ...
                        try {
                            if (window.SaveManager && typeof SaveManager.saveSlot === 'function') {
                                try { await SaveManager.saveSlot(i, saveObj); } catch (e) { localStorage.setItem(`dungeons_edge_save_slot_${i}`, JSON.stringify(saveObj)); }
                            } else {
                                localStorage.setItem(`dungeons_edge_save_slot_${i}`, JSON.stringify(saveObj));
                            }
                        } catch (e) {
                            try { localStorage.setItem(`dungeons_edge_save_slot_${i}`, JSON.stringify(saveObj)); } catch (e) {}
                        }
                    } catch (e) {}
                    try { localStorage.setItem('dungeons_edge_last_slot', i); } catch (e) {}
                        ...
                        if (el) {
                            if (el.tagName === 'BUTTON') {
                                ...
                                if (row < 3 && slotRows[row][1]) {
                                    ...
                                    try {
                                        const saveObj = { nome: slotName, criadoEm: Date.now() };
                                        try { saveObj.dinheiro = (typeof money !== 'undefined') ? Number(money) : 0; } catch (e) { saveObj.dinheiro = 0; }
                                        try { saveObj.profundidade = (typeof salvoprofundidade !== 'undefined') ? Math.floor(Number(salvoprofundidade) || 0) : 0; } catch (e) { saveObj.profundidade = 0; }
                                        localStorage.setItem(`dungeons_edge_save_slot_${row+1}`, JSON.stringify(saveObj));
                                    } catch (e) {}
                                    localStorage.setItem('dungeons_edge_last_slot', row+1);
                                    if (slotOverlay && slotOverlay.parentNode) {
                                        if (typeof slotOverlay._removeBlockEnter === 'function') slotOverlay._removeBlockEnter();
                                        if (typeof slotOverlay._removeKeyboardNav === 'function') slotOverlay._removeKeyboardNav();
                                        slotOverlay.parentNode.removeChild(slotOverlay);
...
function openLoadMenu() {
    ...
    async function getSlot(i) {
        ...
        try {
            const raw = localStorage.getItem(`dungeons_edge_save_slot_${i}`);
    ...
    btns.appendChild(loadBtn);
        ...
        (async (idx, infoEl, lbtn) => {
            ...
            lbtn.addEventListener('click', async function(e) {
                e.stopPropagation();
                try { localStorage.setItem('dungeons_edge_last_slot', idx); } catch (e) {}
