(function(){
  ...
  _tutorial_End: false,
    ...
    _uiText: {
      ...
      previewSelectText: 'À direita da prévia do seu personagem que vimos anteriormente há um Destaque na habilidade. Passe o mouse ou selecione com as setas para ver a habilidade do personagem.',
      previewExplanationTitle: 'Introdução às Habilidades',
      previewExplanationText: 'Essa é uma previsão de sua habilidade específica. Fique de olho nessa área, mas previsões iram aparecer.',
      buttonsTitle: 'Os Botões Principais',
      buttonsText: 'Vamos falar agora dos 3 botões na loja. Esses já contém os nomes explícitos do que eles fazem, mas vamos passar por eles para dar uma ideia melhor.',
      ...
      characterBtnText: 'Esse botão de selecionar personagem deixa você selecionar entre personagens desbloqueados e ver suas estatísticas específicas. No momento você só tem um personagem jogável.',
      ...
      depthNextText: 'A profundidade até o próximo segredo será seu objetivo. Chegue até essa profundidade para liberar novos itens secretos na loja que irão te ajudar a chegar mais fundo e descobrir mais segredos. Fique de olho nessa informação para saber qual é seu próximo objetivo de profundidade.',
      ...
      moneyText: 'Aqui você vê quanto dinheiro você tem. O dinheiro é usado para compras na loja. Colete mais explorando a dungeon nas suas tentativas!',
      characterPreviewTitle: 'Visualização do Personagem',
      characterPreviewText: 'Esta é uma prévia do seu personagem. Ele mostra as estatísticas atuais dele. Você pode ver sua vida, capacidade de pulo e velocidade. Elas podem ser melhoradas em personagens específicos com itens da loja!',
      ...
      storeBuyingIntroductionTitle: 'Introdução aos Itens na Loja',
      storeBuyingIntroductionText: 'À esquerda da sua tela, na parte de baixo da prévia do seu personagem, está a localização de seleção de itens da loja. Antes não havia nada, mas agora com seu progresso na dungeon, um novo item foi desbloqueado — dê uma olhada nele.',
      storeBuyingPurchaseTitle: 'Comprar Item',
      storeBuyingPurchaseText: 'Agora compre esse item com o teclado ou mouse para darmos continuidade.',
      storeBuyingPurchaseHint: 'Compre o item para continuar',
      storeBuyingFinalTitle: 'Últimas etapas',
      storeBuyingFinalText: 'Ótimo! Após comprar, algo novo apareceu — isso é normal. Certos itens, quando comprados pela primeira vez, desbloqueiam outros na loja (por exemplo: "Botas do Vento" com a compra do "Cinto Relâmpago"). Sempre olhe os novos itens; eles podem ser necessários para chegar mais fundo na dungeon. Fique atento a etiquetas como "NEW" ou "LEIA" no canto superior direito dos itens, que indicam itens recém-desbloqueados. Verifique também os status do seu personagem para ver se o item adquirido trouxe alguma melhoria, já que eles são equipados automaticamente ao comprar.',
      storeBuyingCompleteTitle: 'Boa sorte!',
      storeBuyingCompleteText: 'Com isso, o tutorial foi concluído. Ainda há bem mais a se ver, mas com esse básico você conseguirá se virar para descobrir. Agora é hora de aplicar o que aprendeu. Lembre-se: a jornada é tão importante quanto o destino. Continue descendo e aprendendo a cada passo.',
      purchaseInstructionsTitle: 'Explicação de itens na Loja',
      purchaseInstructionsText: 'A caixa que aparece ao selecionar o Cinto Relâmpago é uma caixa de descrição. Nela são apresentados os requisitos específicos de compra e o preço (ambos podem variar a cada aquisição), assim como os efeitos que o item aplica ao seu personagem automaticamente após a compra. Você conseguirá visualizar essa caixa sempre que passar o mouse sobre o item ou selecioná-lo com o teclado.',
      ...
      dungeonInteractionText: 'Agora vamos aprender sobre a dungeon. Selecione o botão \'Ir para a Dungeon\' usando o mouse ou o teclado para continuarmos.',
      failedDepth50Title: 'Quase lá',
      failedDepth50Text: 'SE VC ESTA LENDO ISSO O CODIGO DEU ERRO EM ALGUM LUGAR O POHAAAA considere isso um easter egg seu pola',
    }, 
...
if (hasOverlay) {
        ...
        try {
          try {
            ...
            if (last && window.SaveManager && typeof SaveManager.loadSlot === 'function' && typeof SaveManager.saveSlot === 'function') {
              ...
              // When marking the tutorial as seen we want to record that the
              // dungeon tutorial should no longer be paused or movement-only.
              // Save explicit `false` values here so the "seen" state is clear.
              slot.tutorials.tutorial_paused = false;
              slot.tutorials.tutorial_movement_only = false;
            ...
            if (typeof gameState !== 'undefined' && gameState === 'loja' && Tutorial.showShopTutorial) {
              if (Tutorial._stage === 'consent') {
                ...
                if (keyLower === 'y') {
                  ...
                  // Player declined the tutorial: ensure dungeon tutorial flags are
                  // recorded as not-paused / not movement-only and mark seen.
                  try { DungeonTutorial._tutorialPaused = false; } catch (e) {}
                  try { DungeonTutorial._tutorialMovementOnly = false; } catch (e) {}
                  try { DungeonTutorial._stage = 'Dmarkseen'; } catch (e) {}
                  try { Tutorial._stage = 'markseen'; } catch (e) {}
                  try { if (typeof DungeonTutorial._dungeonmarkSeen === 'function') DungeonTutorial._dungeonmarkSeen(); } catch (e) {}
                  try { Tutorial._saveProgress(); } catch (e) {}
  ...
  try {
    (function() {
      ...
      // Also intercept DungeonTutorial pause/movement-only flags so changes
      // to `tutorial_paused` / `tutorial_movement_only` can be observed in logs.
      try {
        if (typeof window.DungeonTutorial !== 'undefined' && window.DungeonTutorial) {
          const dt = window.DungeonTutorial;
          try { dt._tutorialPaused_backup = dt._tutorialPaused; } catch (e) { dt._tutorialPaused_backup = undefined; }
          try {
            Object.defineProperty(dt, '_tutorialPaused', {
              configurable: true,
              enumerable: true,
              get: function() { return this._tutorialPaused_backup; },
              set: function(v) {
                try { console.log('[DungeonTutorial] _tutorialPaused ->', v, '\nStack:', (new Error()).stack); } catch (e) {}
                this._tutorialPaused_backup = v;
              }
            });
          } catch (e) {}

          try { dt._tutorialMovementOnly_backup = dt._tutorialMovementOnly; } catch (e) { dt._tutorialMovementOnly_backup = undefined; }
          try {
            Object.defineProperty(dt, '_tutorialMovementOnly', {
              configurable: true,
              enumerable: true,
              get: function() { return this._tutorialMovementOnly_backup; },
              set: function(v) {
                try { console.log('[DungeonTutorial] _tutorialMovementOnly ->', v, '\nStack:', (new Error()).stack); } catch (e) {}
                this._tutorialMovementOnly_backup = v;
              }
            });
          } catch (e) {}
        }
